import { validatePyTealCode, autoFixPyTealCode } from "./pyteal-validator"
import { findMatchingTemplate } from "./pyteal-templates"
import { findRelevantConcepts } from "./pyteal-knowledge-base"
import { validateAndCorrectResponse } from "./response-validator"

/**
 * Enhanced response validator specifically for PyTeal code
...
export async function enhancedValidateAndCorrectResponse(
  ...
  for (const codeBlock of codeBlocks) {
    // Validate the PyTeal code
    const validationResult = validatePyTealCode(codeBlock.code)

    if (!validationResult.isValid || validationResult.warnings.length > 0) {
      // Try to fix the code
      const fixedCode = autoFixPyTealCode(codeBlock.code, validationResult)
